...
/**
 * Service responsible for interacting with Riot's static content APIs (Data
 * Dragon) and
 * the community-driven CommunityDragon dataset. The service centralizes every
 * read-only
 * lookup for champions, localized assets, curated tooltips and sanitized HTML
 * snippets.
 *
 * Responsibilities include:
 * <ul>
 * <li>Fetching and caching Data Dragon versions, champion lists and champion
 * details</li>
 * <li>Merging curated local JSON bundles with official Riot data for richer
 * tooltips</li>
 * <li>Sanitizing HTML fragments so that tooltips can be rendered safely in the
 * browser</li>
 * <li>Falling back to CommunityDragon data when Riot's payloads omit numeric
 * values</li>
 * <li>Providing helper utilities to normalize locales, tokens and champion
 * identifiers</li>
 * </ul>
 */
@Service
public class DataDragonService {
    ...
    // Provider used to obtain the proxied (AOP-aware) instance for
    // cacheable/self-invoking methods
    ...
    // In-memory cache of curated tooltip bundles per locale to avoid repeated disk
    // access
    private final ConcurrentMap<String, Map<String, LocalAbilities>> curatedChampionCache = new ConcurrentHashMap<>();

    // Conservative HTML sanitization policy for tooltips/descriptions from
    // DDragon/CDragon.
    // Allow common inline formatting and a small set of known game-specific tags
    // used in tooltips
    // (e.g., <magicDamage>, <physicalDamage>, <trueDamage>, <status>, <br>),
    // without any attributes.
    private static final PolicyFactory TOOLTIP_POLICY = new HtmlPolicyBuilder()
            .allowElements("b", "strong", "i", "em", "u", "small", "sup", "sub", "br", "span")
            .allowElements("magicdamage", "physicaldamage", "truedamage", "status")
            // No attributes/URLs allowed; compose with basic formatting sanitizer for
            // safety
            .toFactory()
    ...
    private static final Pattern REPEATED_NUMBER_PATTERN = Pattern
            .compile("(?i)\\b(\\d+(?:[\\.,]\\d+)?)(?:[\\s\\u00A0]+)\\1\\b");
    private static final Pattern SECONDS_EN_PATTERN = Pattern
            .compile("(?i)for(?:[\\s\\u00A0]+)(?:<[^>]+>(?:[\\s\\u00A0])*)*(second(?:s|\\(s\\))?|sec(?:\\.|onds?)?)");
    private static final Pattern SECONDS_DE_PATTERN = Pattern.compile("(?i)(?:<[^>]+>(?:[\\s\\u00A0])*)*sekunde(n)?");
    private static final Pattern NUMBER_BEFORE_SECONDS_PATTERN = Pattern
            .compile(".*\\d+(?:[\\s\\u00A0])*(?:second(?:s|\\(s\\))?|sec(?:\\.|onds?)?).*");
    private static final Pattern NUMBER_BEFORE_SEKUNDE_PATTERN = Pattern
            .compile(".*\\d+(?:[\\s\\u00A0])*sekunde(n)?.*");
    private static final Pattern TWO_NUMBERS_SECONDS_PATTERN = Pattern.compile(
            "(?i)\\b(\\d+(?:[\\.,]\\d+)?)\\s*\\u00A0?\\s+(\\d+[\\.,]\\d+)\\s*\\u00A0?\\s*(sekunde(n)?|second(s)?)\\b");
    private static final Pattern TWO_SINGLE_DIGITS_SECONDS_PATTERN = Pattern
            .compile("(?i)\\b(\\d)\\s*\\u00A0?\\s+(\\d)\\s*\\u00A0?\\s*(sekunde(n)?|second(s)?)\\b");
    private static final Pattern TWO_NUMBERS_TAGS_SECONDS_PATTERN = Pattern.compile(
            "(?is)\\b(\\d)(?:[\\s\\u00A0]+|<[^>]+>)+(\\d)(?:[\\s\\u00A0]+|<[^>]+>)+(sekunde(n)?|second(s)?)\\b");
    private static final Pattern TWO_DECIMALS_TAGS_SECONDS_PATTERN = Pattern.compile(
            "(?is)\\b(\\d+(?:[\\.,]\\d+)?)(?:[\\s\\u00A0]+|<[^>]+>)+(\\d+[\\.,]\\d+)(?:[\\s\\u00A0]+|<[^>]+>)+(sekunde(n)?|second(s)?)\\b");
    private static final Pattern MAGIC_DAMAGE_PATTERN = Pattern
            .compile(".*\\d+(?:[\\.,]\\d+)?[^\\n\\r]{0,40}magic damage.*");
    private static final Pattern MAGISCHEN_SCHADEN_PATTERN = Pattern
            .compile(".*\\d+(?:[\\.,]\\d+)?[^\\n\\r]{0,40}magischen schaden.*");
    private static final Pattern VERRINGERTEN_SCHADEN_PATTERN = Pattern
            .compile(".*\\d+(?:[\\.,]\\d+)?[^\\n\\r]{0,40}verringerten normalen schaden.*");
    private static final Pattern NUMBER_PARSER_PATTERN = Pattern.compile("(\\d+(?:[\\.,]\\d+)?)");
    private static final Pattern CDRAGON_TOKEN_PATTERN = Pattern.compile("@([A-Za-z0-9_.:]+)@");
    private static final Pattern PERCENT_PATTERN = Pattern.compile(".*\\d+(?:[\\s\\u00A0])*%.*");

    /**
     * Constructs the service with HTTP client dependencies and runtime
     * configuration.
     *
     * @param riotApiHttpClient shared HTTP client configured with tracing/executor
     *                          propagation
     * @param defaultLocale     default locale configured through application
     *                          properties
     * @param userAgent         descriptive user agent sent to Data Dragon /
     *                          CommunityDragon
     * @param selfProvider      ObjectProvider that allows the bean to call its own
     *                          proxied methods
     */
    public DataDragonService(HttpClient riotApiHttpClient,
        ...
        this.userAgent = (userAgent == null || userAgent.isBlank()) ? "SummonerAPI/2.0 (github.com/zerox80/SummonerAPI)"
                : userAgent;
        this.selfProvider = selfProvider;
    }

    /**
     * Retrieves either the proxied Spring bean (to honor caching annotations) or
     * falls back
     * to the current instance when running outside the application context (e.g.,
     * tests).
     */
    ...
    /**
     * Helper that counts digits within a string to heuristically detect missing
     * numbers
    ...
    /**
     * Collects tooltip tokens (damage numbers, cooldown strings, etc.) from a Data
     * Dragon
     * spell node so that they can be re-used across multiple abilities when
     * rendering rich
     * tooltips. The resulting map mirrors Riot's token names (e.g., e1, cooldown,
     * slowamount).
     */
    private void collectGlobalDDragonValues(JsonNode spellNode, Map<String, String> out) {
        if (spellNode == null || out == null)
            return;
        // datavalues
        ...
        if (datavalues != null && datavalues.isObject()) {
            datavalues.fields().forEachRemaining(e -> {
                ...
                if (!k.isBlank() && !v.isBlank())
                    out.putIfAbsent(k.toLowerCase(Locale.ROOT), v);
            });
        }
        ...
        if (!cooldown.isBlank())
            out.putIfAbsent("cooldown", cooldown);
        String cost = spellNode.path("costBurn").asText("");
        if (!cost.isBlank())
            out.putIfAbsent("cost", cost);
        // Sometimes tokens like slowamount exist in other spells' datavalues in some
        // champs; already covered above.
    }

    /**
     * Normalizes champion identifiers by stripping non alphabetic characters and
     * limiting
     * length. This prevents malformed champion IDs from being used in downstream
     * lookups.
     */
    ...
    /**
     * Attempts to map a user-provided champion identifier to the canonical ID
     * exposed by
     * Data Dragon by comparing both champion IDs and champion names in a
     * case-insensitive way.
     *
     * @return The canonical champion ID if a match is found, otherwise
     *         {@code null}.
     */
    private String resolveCanonicalChampionId(String candidateId, String version, String localeTag)
        ...
        for (ChampionSummary summary : self().getChampionSummariesCached(version, localeTag)) {
            ...
            if (summaryIdSanitized == null)
                continue;
            if (summaryIdSanitized.equalsIgnoreCase(candidateId)
                    || summaryIdSanitized.toLowerCase(Locale.ROOT).equals(lowerCandidate)) {
                return summary.getId();
            }
            String summaryNameSanitized = sanitizeChampionIdBasic(summary.getName());
            if (summaryNameSanitized != null && (summaryNameSanitized.equalsIgnoreCase(candidateId)
                    || summaryNameSanitized.toLowerCase(Locale.ROOT).equals(lowerCandidate))) {
                return summary.getId();
            }
        }
        return null;
    ...
    /**
     * Loads the complete list of Data Dragon patch versions and caches the result
     * for twelve hours.
     * The first entry is stored as {@link #lastKnownVersion} for subsequent
     * fallbacks.
     */
    ...
    /**
     * Convenience accessor that returns the newest Data Dragon version. When the
     * remote
     * endpoint cannot be reached, the method falls back to
     * {@link #lastKnownVersion}.
     */
    ...
    /**
     * Maps arbitrary {@link Locale} inputs (browser language, HTTP headers, etc.)
     * to the
     * locale identifiers expected by Data Dragon (e.g., {@code de_DE},
     * {@code en_US}).
     */
    ...
    /**
     * Returns all champion summaries for the newest patch, localized to the
     * provided locale.
     ...
     * The list is cached per version/locale combination because the upstream
     * payload is static.
     */
    @Cacheable(cacheNames = "ddragonChampionList", key = "#version + '|' + #localeTag")
    public List<ChampionSummary> getChampionSummariesCached(String version, String localeTag)
            throws IOException, InterruptedException {
    ...
    /**
     * Fetches the full detail payload (lore, tags, spells, passive) for a champion
     * in a given locale.
     * Applies curated overrides when available and gracefully falls back to English
     * resources.
     */
    ...
    /**
     * Cacheable variant of {@link #getChampionDetail(String, Locale)} that exposes
     * deterministic
     * keys (champion/version/locale) so downstream services can render champion
     * pages instantly.
     */
    ...
    public ChampionDetail getChampionDetailCached(String championId, String version, String localeTag)
            throws IOException, InterruptedException {
    ...
    /**
     * Small immutable holder containing lore, passive and spell details either
     * loaded from
     * curated JSON bundles or synthesized from Data Dragon payloads. Keeps track of
     * the locale
    ...
    /**
     * Loads champion abilities from bundled resources (when available) or from
     * per-champion
     * JSON files stored under {@code abilities/<locale>/<champion>.json}. These
     * curated files
        ...
        try {
            ...
            if (sArr != null && sArr.isArray()) {
                for (JsonNode s : sArr) {
                    ...
                    if (s.hasNonNull("cooldown"))
                        sum.setCooldown(s.path("cooldown").asText(""));
                    if (s.hasNonNull("cost"))
                        sum.setCost(s.path("cost").asText(""));
                    if (s.hasNonNull("range"))
                        sum.setRange(s.path("range").asText(""));
                    if (s.hasNonNull("damage"))
                        sum.setDamage(s.path("damage").asText(""));
                    if (s.hasNonNull("scaling"))
                        sum.setScaling(s.path("scaling").asText(""));
                    if (s.has("notes") && s.path("notes").isArray()) {
                        java.util.List<String> notes = new java.util.ArrayList<>();
                        for (JsonNode n : s.path("notes"))
                            notes.add(n.asText(""));
                        sum.setNotes(notes);
                    }
                    spells.add(sum);
    ...
    /**
     * Attempts to resolve a champion entry from the curated bundle (used for
     * locales that
     * would otherwise rely solely on Riot's payload). Results are cached in-memory
     * per locale.
     */
    private LocalAbilities loadCuratedAbilitiesFromBundle(String championId, String localeTag) {
        ...
        Map<String, LocalAbilities> byChampion = curatedChampionCache.computeIfAbsent(cacheKey,
                key -> loadCuratedLocaleMap(canonicalLocale));
        if (byChampion == null || byChampion.isEmpty())
            return null;
        return byChampion.get(championId.toLowerCase(Locale.ROOT));
    }

    /**
     * Loads and deserializes the curated bundle ({@link #CURATED_BUNDLE_NAME}) for
     * a specific locale.
     *
     * @return map of lower-cased champion IDs to {@link LocalAbilities} or
    ...
    private LocalAbilities parseCuratedChampionNode(JsonNode node, String localeTag) {
        ...
        if (spellsNode != null && spellsNode.isArray()) {
            for (JsonNode spellNode : spellsNode) {
                if (spellNode == null || !spellNode.isObject())
                    continue;
    ...
    /**
     * Converts locale tags to the canonical underscore format used by bundled
     * assets.
     */
    ...
    /**
     * Parses the standard Data Dragon champion payload and builds
     * {@link LocalAbilities}
     * from it by extracting passive/spell details, resolving tooltip tokens and
     * sanitizing
     * the resulting HTML.
     */
    ...
    /**
     * Returns the first non-blank string, preferring {@code primary} and falling
     * back to {@code fallback}.
     */
    ...
    /**
     * Resolves Riot's templated tooltip strings by substituting placeholders such
     * as
     * {@code {{ e1 }}} with concrete damage/cooldown numbers. The method merges
     * per-spell values with the optional {@code globalValues} map to mimic Riot's
     * client.
     */
    private String renderSpellTooltip(JsonNode spellNode, Map<String, String> globalValues) {
        if (spellNode == null || spellNode.isMissingNode())
            return "";
        ...
        // datavalues: keys may match tokens seen in tooltips (e.g., totaldamage,
        // slowduration)
        JsonNode datavalues = spellNode.path("datavalues");
        if (datavalues != null && datavalues.isObject()) {
        ...
        // vars: provide coefficients for a1, a2... Use a readable label with AP/AD when
        // possible.
        JsonNode vars = spellNode.path("vars");
        if (vars != null && vars.isArray()) {
            for (JsonNode v : vars) {
        ...
        // Replace tokens {{ ... }} (support prefixes like spell.xyz:token -> use
        // right-most token)
        // Replace tokens {{ ... }} (support prefixes like spell.xyz:token -> use
        // right-most token)
        ...
        // Heuristic patching: if we still see phrases like "for seconds" or bare "%"
        // without a number,
        // try to inject a plausible value from collected effect burns (e1/e2/...) or
        // datavalues.
        out = patchMissingNumbers(out, values);

        // Some tooltips contain special tags like <status>...</status>; keep as-is for
        // utext
        return out;
    }

    /**
     * Applies heuristic replacements when Riot's templates do not contain the
     * numbers
     * necessary to make a tooltip readable (e.g., "for seconds" without any
     * digits).
     */
    private String patchMissingNumbers(String text, Map<String, String> values) {
        ...
        // Generic cleanup: collapse immediate duplicate numbers (e.g., "1 1" or "0.75
        // 0.75"). Accept NBSP as whitespace.
        result = REPEATED_NUMBER_PATTERN.matcher(result).replaceAll("$1");
        ...
        // Helper to pick candidate from e1/e2/... that looks like a duration (<= 10
        // sec)
        ...
        // English "for seconds" variants (allow tags between words, handle 'second(s)'
        // or 'sec')
        if (secondsCandidate != null && !secondsCandidate.isBlank()) {
            boolean hasNumberSeconds = result
                    .matches(".*\\d+(?:[\\s\\u00A0])*(?:second(?:s|\\(s\\))?|sec(?:\\.|onds?)?).*");
            if (!hasNumberSeconds) {
                // Insert number before the unit, allowing optional inline HTML tags between
                // 'for' and the unit
                result = SECONDS_EN_PATTERN.matcher(result)
                        .replaceFirst("for " + java.util.regex.Matcher.quoteReplacement(secondsCandidate) + " $1");
            }
            // German "Sekunden"/"Sekunde" (allow tags in between)
            boolean hasNumberSek = NUMBER_BEFORE_SEKUNDE_PATTERN.matcher(result).matches();
            if (!hasNumberSek && (result.toLowerCase(Locale.ROOT).contains("sekunde"))) {
                result = SECONDS_DE_PATTERN.matcher(result)
                        .replaceFirst(java.util.regex.Matcher.quoteReplacement(secondsCandidate) + " Sekunden");
            }
        }

        // If two numbers appear before seconds and the second is decimal, keep the
        // decimal one (allow NBSP)
        result = TWO_NUMBERS_SECONDS_PATTERN.matcher(result).replaceAll("$2 $3");
        // Collapse patterns like "1 1 Sekunden" or "1 1 seconds" -> "1.1
        // Sekunden/seconds" (allow NBSP)
        result = TWO_SINGLE_DIGITS_SECONDS_PATTERN.matcher(result).replaceAll("$1.$2 $3");
        // Also allow optional HTML tags between the numbers
        result = TWO_NUMBERS_TAGS_SECONDS_PATTERN.matcher(result).replaceAll("$1.$2 $3");
        // If pattern like '1 <tags> 1.1 Sekunden' occurs, keep the decimal number
        result = TWO_DECIMALS_TAGS_SECONDS_PATTERN.matcher(result).replaceAll("$2 $3");
        ...
        if (dmgCandidate != null && !dmgCandidate.isBlank()) {
            ...
            if (result.contains("<magicDamage></magicDamage>")) {
                result = result.replace("<magicDamage></magicDamage>",
                        "<magicDamage>" + java.util.regex.Matcher.quoteReplacement(dmgCandidate) + "</magicDamage>");
            }
            // German "magischen Schaden" without nearby numbers
            if (result.toLowerCase(Locale.ROOT).contains("magischen schaden")
                    && !MAGISCHEN_SCHADEN_PATTERN.matcher(result).matches()) {
                result = result.replaceFirst("(?i)magischen\\s+schaden",
                        java.util.regex.Matcher.quoteReplacement(dmgCandidate + " magischen Schaden"));
            }
            // English "magic damage" without nearby numbers
            if (result.toLowerCase(Locale.ROOT).contains("magic damage")
                    && !MAGIC_DAMAGE_PATTERN.matcher(result).matches()) {
                result = result.replaceFirst("(?i)magic\\s+damage",
                        java.util.regex.Matcher.quoteReplacement(dmgCandidate + " magic damage"));
            }
        }

        // Small flat number for DE phrase "verringerten normalen Schaden" if missing
        if (result.toLowerCase(Locale.ROOT).contains("verringerten normalen schaden")
                && !VERRINGERTEN_SCHADEN_PATTERN.matcher(result).matches()) {
            String small = pickSmallNumberCandidate(values, 20.0);
            if (small != null && !small.isBlank()) {
                result = result.replaceFirst("(?i)um\\s+(?:<[^>]+>\\s*)*verringerten\\s+normalen\\s+schaden",
                        java.util.regex.Matcher.quoteReplacement("um " + small + " verringerten normalen Schaden"));
            }
        }

        // Normalize spaces around NBSP: collapse sequences like " \u00A0" or "\u00A0 "
        // to a single NBSP
    ...
    /**
     * Picks a reasonable damage string (e.g., "80/110/140") from the supplied token
     * map by
     * preferring entries that look like damage/eN tokens and discarding absurd
     * values.
     */
    private String pickDamageCandidate(Map<String, String> values) {
        ...
        for (Map.Entry<String, String> e : values.entrySet()) {
            ...
            if (!(k.matches("e\\d+") || k.contains("damage") || k.contains("dmg") || k.contains("magic")))
                continue;
    ...
    /**
     * Selects the smallest positive value below {@code max} – handy for
     * cooldown/second heuristics.
     */
        ...
        for (Map.Entry<String, String> e : values.entrySet()) {
            ...
            for (Double d : nums) {
                if (d != null && d > 0 && d <= max) {
                    ...
                    if (d < bestVal) {
                        bestVal = d;
                        best = (d % 1 == 0 ? String.valueOf(d.intValue()) : formatCoeff(d));
                    }
                }
            }
        }
    ...
    /**
     * Finds a slash-separated value whose parsed numbers satisfy the provided
     * validator.
     */
    private String pickJoinedCandidate(Map<String, String> values,
            java.util.function.Predicate<java.util.List<Double>> validator) {
        if (values == null || values.isEmpty())
            return null;
        String best = null;
        double bestAvg = -1.0;
        for (Map.Entry<String, String> e : values.entrySet()) {
            ...
            if (k == null || v == null)
                continue;
            if (!k.matches("e\\d+|.*duration.*|.*time.*|.*stun.*|.*root.*|.*immobil.*|.*sleep.*|.*knock.*|.*freeze.*"))
                continue;
            java.util.List<Double> nums = parseNumbers(v);
    ...
    /**
     * Attempts to find a candidate that clearly represents a percentage/slow
     * modifier.
     */
    private String pickPercentCandidate(Map<String, String> values) {
        ...
        for (Map.Entry<String, String> e : values.entrySet()) {
            ...
            if (k == null || v == null)
                continue;
            if (!(k.contains("percent") || k.contains("slow") || k.contains("movement") || k.contains("speed")
                    || k.contains("ms") || k.contains("hp") || k.contains("health")))
                continue;
            java.util.List<Double> nums = parseNumbers(v);
            if (nums.isEmpty())
                continue;
    ...
    /**
     * Parses every number (accepting commas or dots) from a combined string and
     * returns them as doubles.
     */
    private java.util.List<Double> parseNumbers(String joined) {
        java.util.List<Double> list = new java.util.ArrayList<>();
        if (joined == null || joined.isBlank())
            return list;
        java.util.regex.Matcher m = NUMBER_PARSER_PATTERN.matcher(joined);
        while (m.find()) {
    ...
    /**
     * Normalizes Riot/CommunityDragon placeholder tokens by removing namespaces and
     * lower-casing.
     */
    private String normalizeToken(String token) {
        ...
        // Remove any namespace prefixes (e.g., spell.glacialstorm:slowamount ->
        // slowamount)
    ...
    /**
     * Detects whether a tooltip still looks unresolved (missing numbers, stray
     * placeholders, etc.).
     */
    private boolean looksIncompleteTooltip(String text, String locale) {
        ...
        if (lc.contains("{{") || lc.contains("}}") || lc.indexOf('@') >= 0)
            return true;
        // No digits at all -> likely missing numbers
        if (!lc.matches(".*\\d+.*"))
            return true;
        // Percent without preceding number
        if (lc.contains("%") && !PERCENT_PATTERN.matcher(lc).matches())
            return true;
        // "for seconds" or localized variants without a number just before the unit
        if (SECONDS_EN_PATTERN.matcher(lc).find() && !NUMBER_BEFORE_SECONDS_PATTERN.matcher(lc).matches())
            return true;
        if (SECONDS_DE_PATTERN.matcher(lc).find() && !NUMBER_BEFORE_SEKUNDE_PATTERN.matcher(lc).matches())
            return true;
        return false;
    }

    /**
     * Formats coefficients for display (trim trailing zeros) so the UI shows "0.6"
     * instead of "0.600000".
     */
    ...
    /**
     * Converts Riot's link identifiers to human readable stat labels used in
     * tooltips.
     */
    ...
    /**
     * Downloads CommunityDragon spell payloads and resolves their {@code @Token@}
     * placeholders
     * to plain text. CommunityDragon often contains the fully substituted values
     * even when
     * Data Dragon does not, making it a reliable fallback for missing numbers.
     */
    private List<String> fetchCDragonResolvedTooltips(int championKey, String ddragonLocale)
            throws IOException, InterruptedException {
        String loc = cdragonLocale(ddragonLocale);
        String url = "https://raw.communitydragon.org/latest/plugins/rcp-be-lol-game-data/global/" + loc
                + "/v1/champions/" + championKey + ".json";
        JsonNode node = getJson(url);
        ...
        // Also fetch default-locale (en) as a robust fallback when localized strings
        // miss numbers
        JsonNode defNode = null;
        JsonNode defSpells = null;
        try {
            String defUrl = "https://raw.communitydragon.org/latest/plugins/rcp-be-lol-game-data/global/default/v1/champions/"
                    + championKey + ".json";
        ...
        if (defSpells != null && defSpells.isArray()) {
            for (JsonNode sp : defSpells) {
                ...
                per.forEach((k, v) -> {
                    if (v != null && !v.isBlank())
                        defGlobalValues.putIfAbsent(k, v);
                });
            }
        }
        ...
        for (int i = 0; i < spells.size(); i++) {
            ...
            Matcher m = CDRAGON_TOKEN_PATTERN.matcher(dyn);
            StringBuffer sb = new StringBuffer();
            ...
            while (m.find()) {
                ...
                if (rep == null || rep.isBlank())
                    rep = globalValues.getOrDefault(norm, "");
                if ((rep == null || rep.isBlank()) && defValues != null)
                    rep = defValues.get(norm);
                if ((rep == null || rep.isBlank()) && defGlobalValues != null)
                    rep = defGlobalValues.getOrDefault(norm, "");
                // Fallback: try without trailing digits (e.g., stunduration1 -> stunduration)
                if ((rep == null || rep.isBlank()) && norm.length() > 1
                        && Character.isDigit(norm.charAt(norm.length() - 1))) {
                    String trimmed = norm.replaceAll("\\d+$", "");
                    rep = localValues.get(trimmed);
                    if (rep == null || rep.isBlank())
                        rep = globalValues.getOrDefault(trimmed, "");
                    if ((rep == null || rep.isBlank()) && defValues != null)
                        rep = defValues.get(trimmed);
                    if ((rep == null || rep.isBlank()) && defGlobalValues != null)
                        rep = defGlobalValues.getOrDefault(trimmed, "");
                }
                if (rep == null)
                    rep = "";
                if (rep.isBlank())
            ...
            // If still obviously incomplete (no digits at all), fall back to default
            // dynamic text fully
            if ((!hasDigits || anyTokenMissing) && defSp != null) {
                String dynDef = defSp.path("dynamicDescription").asText("");
                Matcher md = CDRAGON_TOKEN_PATTERN.matcher(dynDef != null ? dynDef : "");
            ...
            // Heuristic patching for missing numbers (e.g., "for seconds") using combined
            // value maps
            Map<String, String> mergedVals = new HashMap<>();
            if (localValues != null)
                mergedVals.putAll(localValues);
            if (globalValues != null)
                mergedVals.putAll(globalValues);
            if (defValues != null)
                mergedVals.putAll(defValues);
            if (defGlobalValues != null)
                mergedVals.putAll(defGlobalValues);
    ...
    /**
     * Extracts the numerous token/value pairs from a CommunityDragon spell
     * definition so that
     * dynamic descriptions can be rendered without hitting the remote API multiple
     * times.
     */
    private Map<String, String> extractCDragonSpellValues(JsonNode spell) {
        ...
        if (sdv != null && sdv.isArray()) {
            for (JsonNode v : sdv) {
                ...
                if (!name.isBlank()) {
                    ...
                    if (val.isBlank())
                        val = joinNumberArray(v.path("valuesPerLevel"));
                    if (!val.isBlank())
                        map.put(name.trim().toLowerCase(Locale.ROOT), val);
                }
            }
        }
        ...
        String cd = spell.path("cooldownCoefficients").isArray() ? joinNumberArray(spell.path("cooldownCoefficients"))
                : spell.path("cooldown").asText("");
        if (cd != null && !cd.isBlank())
            map.put("cooldown", cd);
        String cost = spell.path("costCoefficients").isArray() ? joinNumberArray(spell.path("costCoefficients"))
                : spell.path("cost").asText("");
    ...
    /**
     * Returns a map between champion numeric keys and their square splash URLs for
     * the given locale.
     * Useful for client-side lookups when only the numeric key is known (e.g.,
     * match histories).
     */
    ...
    /**
     * Cache-backed variant of {@link #getChampionKeyToSquareUrl(Locale)} that
     * prevents redundant
    ...
    public Map<Integer, String> getChampionKeyToSquareUrlCached(String version, String localeTag)
            throws IOException, InterruptedException {
        String url = DDRAGON_BASE + "/cdn/" + version + "/data/" + localeTag + "/champion.json";
        JsonNode root = getJson(url);
        JsonNode data = root.path("data");
    ...
    /**
     * Adds "name" + "values" style entries from nested CommunityDragon structures
     * to a map.
     */
        ...
        if (node.isArray()) {
            for (JsonNode v : node) {
                ...
                if (!name.isBlank()) {
                    ...
                    if (val.isBlank())
                        val = joinNumberArray(v.path("valuesPerLevel"));
                    if (!val.isBlank())
                        out.put(name.trim().toLowerCase(Locale.ROOT), val);
                }
            }
            ...
            node.fields().forEachRemaining(e -> {
                ...
                if (v.isArray()) {
                    ...
                    if (!val.isBlank())
                        out.put(key.trim().toLowerCase(Locale.ROOT), val);
                } else if (v.isObject()) {
                    String name = v.path("name").asText("");
                    ...
                    if (val.isBlank())
                        val = joinNumberArray(v.path("valuesPerLevel"));
                    if (val.isBlank())
                        val = joinNumberArray(v.path("coefficients"));
                    if (val.isBlank())
                        val = joinNumberArray(v.path("mValue"));
                    if (val.isBlank())
                        val = joinNumberArray(v.path("value"));
                    if (val.isBlank() && v.isNumber())
                        val = joinNumberArray(v);
                    if (!val.isBlank())
                        out.put(outKey.trim().toLowerCase(Locale.ROOT), val);
    ...
    /**
     * Utility that turns a numeric/textual JSON array into a slash-separated string
     * (e.g., "10/20/30").
     */
    private String joinNumberArray(JsonNode arr) {
        ...
        if (arr.isArray()) {
            ...
            for (JsonNode n : arr) {
                if (n.isNumber())
                    parts.add(formatCoeff(n.asDouble()));
                else if (n.isTextual())
                    parts.add(n.asText());
            }
    ...
    private String cdragonLocale(String ddragonLocale) {
        if (ddragonLocale == null || ddragonLocale.isBlank())
            return "en_us";
    ...
    /**
     * Applies hand-crafted tooltip fixes for champions whose data is notoriously
     * incomplete
     * (for example, Anivia or localized Amumu tooltips). The overrides are language
     * aware and
     * therefore only touch tooltips for the relevant locales.
     */
    private void applyChampionSpecificTooltipOverrides(String championId, String ddragonLocale,
            ...
            if (spells.size() >= 4) {
                ...
                if (q.getTooltip() == null || !q.getTooltip().matches(".*\\d+.*")) {
                    if (isGerman) {
                        ...
                        q.setTooltip(
                                "Anivia fires a massive chunk of ice, dealing <magicDamage>60/85/110/135/160 (+45% AP) magic damage</magicDamage>, <keywordMajor>Chilling</keywordMajor> enemies for 3 seconds and <status>Slowing</status> them by 20/30/40%. At max range the ice detonates, <status>Stunning</status> for 1.1/1.2/1.3/1.4/1.5 seconds and dealing an additional <magicDamage>60/85/110/135/160 (+45% AP) magic damage</magicDamage>.<br><br>Can be <recast>recast</recast> to detonate early.");
                    }
                }
                if (w.getTooltip() == null || !w.getTooltip().matches(".*\\d+.*")) {
                    if (isGerman) {
                        w.setTooltip(
                                "Anivia beschwört eine Eiswand herauf, die <b>5</b> Sekunden lang besteht und <b>400/500/600/700/800</b> Einheiten breit ist.");
                    } else if (isEnglish) {
                        w.setTooltip(
                                "Anivia summons a wall of ice that lasts <b>5</b> seconds and is <b>400/500/600/700/800</b> units wide.");
                    }
                }
                if (e.getTooltip() == null || !e.getTooltip().matches(".*\\d+.*")) {
                    if (isGerman) {
                        e.setTooltip(
                                "Anivia entfesselt einen frostigen Windstoß und verursacht <magicDamage>60/90/120/150/180 (+60% AP) magischen Schaden</magicDamage>. Gegen <keywordMajor>unterkühlte</keywordMajor> Ziele verursacht sie stattdessen <magicDamage>120/180/240/300/360 (+120% AP) magischen Schaden</magicDamage>.");
                    } else if (isEnglish) {
                        e.setTooltip(
                                "Anivia blasts a freezing gust, dealing <magicDamage>60/90/120/150/180 (+60% AP) magic damage</magicDamage>. Against <keywordMajor>Chilled</keywordMajor> targets, she instead deals <magicDamage>120/180/240/300/360 (+120% AP) magic damage</magicDamage>.");
                    }
                }
                if (r.getTooltip() == null || !r.getTooltip().matches(".*\\d+.*")) {
                    if (isGerman) {
                        r.setTooltip(
                                "<toggle>Aktivierbar:</toggle> Beschwört einen Eissturm, der Gegner um <b>20/30/40&nbsp;%</b> <status>verlangsamt</status> und <magicDamage><b>40/60/80</b> magischen Schaden pro Sekunde</magicDamage> verursacht. Der Sturm wächst über <b>1.5</b> Sekunden.<br><br>Voll ausgebildet <keywordMajor>unterkühlt</keywordMajor> der Sturm, <status>verlangsamt</status> um <b>40/50/60&nbsp;%</b> und verursacht <magicDamage><b>80/120/160</b> magischen Schaden pro Sekunde</magicDamage>. Kosten: <b>60</b> + <b>40/50/60</b> Mana pro Sekunde.");
                    } else if (isEnglish) {
                        r.setTooltip(
                                "<toggle>Toggle:</toggle> Calls a storm that <status>Slows</status> by <b>20/30/40%</b> and deals <magicDamage><b>40/60/80</b> magic damage per second</magicDamage>, growing over <b>1.5</b> seconds.<br><br>When fully formed, it <keywordMajor>Chills</keywordMajor>, <status>Slows</status> by <b>40/50/60%</b> and deals <magicDamage><b>80/120/160</b> magic damage per second</magicDamage>. Cost: <b>60</b> + <b>40/50/60</b> mana per second.");
                    }
                }
            }
        }
        ...
        if (("Amumu".equalsIgnoreCase(id) || "32".equals(id)) && isGerman) {
            if (spells.size() >= 1) {
                ...
                if (q != null) {
                    ...
                    // Robust: collapse any number/tag sequence before 'Sekunden' to exactly '1.1
                    // Sekunden'
                    tip = tip.replaceAll("(?is)(?:\\d+(?:[\\.,]\\d+)?(?:[\\s\\u00A0]+|<[^>]+>)+)+sekunde(n)?",
                            "1.1 Sekunden");
                    // Fallback: if 'Sekunden' present without number immediately before, enforce
                    // 1.1
                    if (tip.toLowerCase(Locale.ROOT).contains("sekunden") && !tip.matches(
                            ".*\\d+(?:[\\.,]\\d+)?(?:[\\s\\u00A0])*(?:<[^>]+>(?:[\\s\\u00A0])*)*sekunde(n)?.*")) {
            ...
            if (spells.size() >= 4) {
                ...
                if (r != null) {
                    ...
                    if (tip.toLowerCase(Locale.ROOT).contains("sekunden")
                            && !tip.matches(".*\\d+(?:[\\.,]\\d+)?\\s*sekunde(n)?.*")) {
    ...
    /**
     * Cache-aware helper that fetches a champion's numeric key while respecting
     * locale-specific
    ...
    public Integer getChampionKeyCached(String championId, String version, String localeTag)
            throws IOException, InterruptedException {
        String cid = sanitizeChampionIdBasic(championId);
        if (cid == null)
            return null;
    ...
    /**
     * Executes an HTTP GET request (preferring HTTP/2) and parses the response body
     * as JSON.
     * When the primary client fails (e.g., HTTP/2 not supported), the method
     * retries once with
    ...
    private JsonNode getJson(String url) throws IOException, InterruptedException {
        ...
        // Fallback: retry once with a simple HTTP/1.1 client (helps in rare ALPN/HTTP2
        // issues)
    ...
    public Map<Integer, com.zerox80.riotapi.model.ItemSummary> getItemsCached(String version, String localeTag)
            throws IOException, InterruptedException {
    ...
    public Map<Integer, String> getRuneStyleNamesCached(String version, String localeTag)
            throws IOException, InterruptedException {
    ...
    public Map<Integer, com.zerox80.riotapi.model.RunePerkInfo> getRunePerkLookup(Locale locale)
            throws IOException, InterruptedException {
        String ver = getLatestVersion();
        String loc = resolveLocale(locale);
    ...
    public Map<Integer, com.zerox80.riotapi.model.RunePerkInfo> getRunePerkLookupCached(String version,
            String localeTag) throws IOException, InterruptedException {
    ...
    public Map<Integer, com.zerox80.riotapi.model.SummonerSpellInfo> getSummonerSpells(Locale locale)
            throws IOException, InterruptedException {
        String ver = getLatestVersion();
        String loc = resolveLocale(locale);
    ...
    public Map<Integer, com.zerox80.riotapi.model.SummonerSpellInfo> getSummonerSpellsCached(String version,
            String localeTag) throws IOException, InterruptedException {
