 ...
 * Provides database access for champion item build statistics aggregated from
 * high-ELO matches.
 *
 * This repository manages data about which items are most commonly built on
 * each champion,
 * tracked per patch version, queue type, and role. Statistics are used to
 * display recommended
...
public interface ChampionItemStatRepository extends JpaRepository<ChampionItemStat, Long> {
     ...
     * Used to check if a specific item build already exists before updating
     * statistics.
     *
     ...
     * @return Optional containing the matching ChampionItemStat if found, empty
     *         otherwise
     */
    Optional<ChampionItemStat> findByChampionIdAndRoleAndPatchAndQueueIdAndItemId(String championId, String role,
            String patch, int queueId, int itemId);

    /**
     * Finds the top 10 most popular item builds for a champion in a specific role.
     * Results are ordered by count (descending) to show most frequently built items
     * first.
     *
     ...
     * @return List of up to 10 item stats ordered by usage count (most popular
     *         first)
     */
    List<ChampionItemStat> findTop10ByChampionIdAndRoleAndPatchAndQueueIdOrderByCountDesc(String championId,
     ...
     *         first)
     */
    List<ChampionItemStat> findTop10ByChampionIdAndPatchAndQueueIdOrderByCountDesc(String championId, String patch,
            int queueId);
    ...
    @org.springframework.data.jpa.repository.Query("DELETE FROM ChampionItemStat c WHERE c.championId = :championId AND c.patch = :patch AND c.queueId = :queueId")
