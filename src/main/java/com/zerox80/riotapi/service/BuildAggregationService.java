 ...
 *
 * Analyzes match data from Emerald/Diamond players to determine popular item
 * builds,
 * runes, and summoner spells for each champion. Supports role-specific
 * aggregation
...
public class BuildAggregationService {
     ...
     * @param transactionManager Injected transaction manager for database
     *                           transactions
     */
    ...
    private List<ChampionItemStat> fetchItems(String championId, String patch, int queueId, String role) {
        ...
        if (StringUtils.hasText(role) && !"ALL".equals(role)) {
            // Database query: Fetch top 10 items for champion+role+patch+queue sorted by
            // frequency
            return itemRepo.findTop10ByChampionIdAndRoleAndPatchAndQueueIdOrderByCountDesc(championId, role, patch,
                    queueId);
        }
        // Database query: Fetch top 10 items for champion+patch+queue (all roles
        // aggregated)
    ...
    private List<ChampionRuneStat> fetchRunes(String championId, String patch, int queueId, String role) {
        ...
        if (StringUtils.hasText(role) && !"ALL".equals(role)) {
            ...
            return runeRepo.findTop10ByChampionIdAndRoleAndPatchAndQueueIdOrderByCountDesc(championId, role, patch,
                    queueId);
        }
        // Database query: Fetch top 10 rune combinations for champion+patch+queue (all
        // roles)
        return runeRepo.findTop10ByChampionIdAndPatchAndQueueIdOrderByCountDesc(championId, patch, queueId);
    }

    /**
     * Private helper method to fetch top 10 summoner spell statistics from
     * database.
     *
     * @param championId The champion identifier
     * @param patch      The game patch version
    ...
    private List<ChampionSpellPairStat> fetchSpells(String championId, String patch, int queueId, String role) {
        // Check if specific role is specified
        if (StringUtils.hasText(role) && !"ALL".equals(role)) {
            // Database query: Fetch top 10 summoner spell pairs for
            // champion+role+patch+queue
            return spellRepo.findTop10ByChampionIdAndRoleAndPatchAndQueueIdOrderByCountDesc(championId, role, patch,
                    queueId);
        }
        // Database query: Fetch top 10 summoner spell pairs for champion+patch+queue
        // (all roles)
     ...
     *
     * Retrieves item, rune, and summoner spell statistics for a champion and
     * enriches
     * them with names, icons, and URLs from DataDragon. If role-specific data is
     * not
    ...
    public ChampionBuildDto loadBuild(String championId, Integer queueId, String role, Locale locale) {
        ...
        List<RuneStatDto> runeDtos = runes.stream().map(s -> {
            ...
            // Create DTO with statistics and metadata
            return new RuneStatDto(s.getPrimaryStyle(), s.getSubStyle(), s.getKeystone(), s.getCount(), s.getWins(),
                    pName, sName, kName, kIcon);
        }).collect(Collectors.toList());
        ...
        List<SpellPairStatDto> spellDtos = spells.stream().map(s -> {
            ...
            // Create DTO with statistics and metadata
            return new SpellPairStatDto(s.getSpell1Id(), s.getSpell2Id(), s.getCount(), s.getWins(), s1Name, s2Name,
                    s1Icon, s2Icon);
        }).collect(Collectors.toList());
     ...
     * retrieves match histories, and aggregates item/rune/spell statistics per
     * role.
     * Results are stored in the database, replacing any existing data for the same
     * champion/patch/queue combination.
     *
     * @param championId         The champion identifier (e.g., "Anivia")
     * @param queueId            The queue type (420=SoloQ, 440=Flex, null defaults
     *                           to 420)
     * @param pagesToScan        Number of league entry pages to scan per
     *                           tier/division
    ...
    public void aggregateChampion(String championId,
        ...
        java.util.Queue<String> summonerIds = new java.util.concurrent.ConcurrentLinkedQueue<>();
            ...
            for (String div : divisions) {
                for (int page = 1; page <= Math.max(1, pagesToScan); page++) {
                    ...
                    futures.add(riot.getEntriesByQueueTierDivision(queueStr, tier, div, page)
                            ...
                            .exceptionally(ex -> {
                                log.warn("Entries fetch failed: {}", ex.toString());
                                return null;
                            }));
                }
            }
        ...
        try {
            ...
            log.warn("Aggregation {}: failed while collecting league entries: {}", championId,
                    ex.getCause() != null ? ex.getCause().toString() : ex.toString());
        }
        log.info("Aggregation {}: fetched {} summoner ids in {}ms", championId, summonerIds.size(),
        ...
        Map<String, Map<Integer, StatCounter>> itemCounts = new java.util.concurrent.ConcurrentHashMap<>();
        Map<String, Map<String, StatCounter>> runeCounts = new java.util.concurrent.ConcurrentHashMap<>();
        Map<String, Map<String, StatCounter>> spellCounts = new java.util.concurrent.ConcurrentHashMap<>();
        ...
        // Process in chunks to avoid unbounded concurrency
        int chunkSize = 50;
        for (int i = 0; i < puuids.size(); i += chunkSize) {
            List<String> chunk = puuids.subList(i, Math.min(i + chunkSize, puuids.size()));
            CompletableFuture<Void> chunkFuture = CompletableFuture.runAsync(() -> {
                List<CompletableFuture<Void>> chunkFutures = new ArrayList<>();
                for (String puuid : chunk) {
                    chunkFutures.add(riot.getMatchIdsByPuuid(puuid, matchesPerSummoner)
                            .thenCompose(ids -> {
                                if (ids == null || ids.isEmpty())
                                    return CompletableFuture.completedFuture(Collections.<MatchV5Dto>emptyList());
                                List<CompletableFuture<MatchV5Dto>> mdFuts = ids.stream().map(riot::getMatchDetails)
                                        .collect(Collectors.toList());
                                return CompletableFuture.allOf(mdFuts.toArray(new CompletableFuture[0]))
                                        .thenApply(v -> mdFuts.stream().map(CompletableFuture::join)
                                                .filter(Objects::nonNull).collect(Collectors.toList()));
                            })
                            .thenAccept(matches -> {
                                for (MatchV5Dto m : matches) {
                                    try {
                                        if (m.getInfo() == null || m.getInfo().getParticipants() == null)
                                            continue;
                                        if (m.getInfo().getQueueId() != q)
                                            continue;
                                        String mv = m.getInfo().getGameVersion();
                                        if (!StringUtils.hasText(mv) || !mv.startsWith(patch + "."))
                                            continue;
                                        long gd = m.getInfo().getGameDuration();
                                        if (gd > 0 && gd < 300)
                                            continue;
                                        for (ParticipantDto p : m.getInfo().getParticipants()) {
                                            if (p == null || p.getChampionId() != championKey.intValue())
                                                continue;
                                            boolean win = p.isWin();
                                            String roleKey = normalizeRole(p.getTeamPosition());

                                            Map<Integer, StatCounter> allItemsBucket = itemCounts.computeIfAbsent("ALL",
                                                    key -> new java.util.concurrent.ConcurrentHashMap<>());
                                            int[] slots = { p.getItem0(), p.getItem1(), p.getItem2(), p.getItem3(),
                                                    p.getItem4(), p.getItem5() };
                                            for (int itemId : slots) {
                                                if (itemId > 0) {
                                                    incrementCounter(allItemsBucket, itemId, win);
                                                    if (roleKey != null) {
                                                        incrementCounter(itemCounts.computeIfAbsent(roleKey,
                                                                key -> new java.util.concurrent.ConcurrentHashMap<>()),
                                                                itemId, win);
                                                    }
                                                }
                                            ...
                                            if (perks != null && perks.getStyles() != null
                                                    && perks.getStyles().size() >= 2) {
                                                PerkStyleDto primary = perks.getStyles().get(0);
                                                PerkStyleDto sub = perks.getStyles().get(1);
                                                int keystone = 0;
                                                if (primary != null && primary.getSelections() != null
                                                        && !primary.getSelections().isEmpty()) {
                                                    keystone = primary.getSelections().get(0).getPerk();
                                                }
                                                String runeKey = primary.getStyle() + "|" + sub.getStyle() + "|"
                                                        + keystone;
                                                Map<String, StatCounter> allRunesBucket = runeCounts.computeIfAbsent(
                                                        "ALL", key -> new java.util.concurrent.ConcurrentHashMap<>());
                                                incrementCounter(allRunesBucket, runeKey, win);
                                                if (roleKey != null) {
                                                    incrementCounter(runeCounts.computeIfAbsent(roleKey,
                                                            key -> new java.util.concurrent.ConcurrentHashMap<>()),
                                                            runeKey, win);
                                                }
                                            }
                                            ...
                                            if (s1 > 0 || s2 > 0) {
                                                ...
                                                Map<String, StatCounter> allSpellsBucket = spellCounts.computeIfAbsent(
                                                        "ALL", key -> new java.util.concurrent.ConcurrentHashMap<>());
                                                incrementCounter(allSpellsBucket, spellKey, win);
                                                if (roleKey != null) {
                                                    incrementCounter(spellCounts.computeIfAbsent(roleKey,
                                                            key -> new java.util.concurrent.ConcurrentHashMap<>()),
                                                            spellKey, win);
                                                }
                                            }
                                        ...
                                        // Defensive: aggregation should never fail the entire job because of a single
                                        // match
                                    }
                                }
                            })
                            .exceptionally(ex -> {
                                log.warn("Aggregation match fetch error: {}", ex.toString());
                                return null;
                            }));
                }
                CompletableFuture.allOf(chunkFutures.toArray(new CompletableFuture[0])).join();
            });
            aggFutures.add(chunkFuture);
        }
        ...
        try {
            ...
            log.warn("Aggregation {} timed out after {} ms. Cancelled {} outstanding tasks.", championId, timeoutMillis,
                    aggFutures.size());
            return;
        ...
        List<ChampionItemStat> itemEntities = itemCounts.entrySet().stream()
                .flatMap(roleEntry -> roleEntry.getValue().entrySet().stream()
                        .map(itemEntry -> newItemEntity(championId, roleEntry.getKey(), patch, q, itemEntry.getKey(),
                                itemEntry.getValue())))
                .collect(Collectors.toList());
        List<ChampionRuneStat> runeEntities = runeCounts.entrySet().stream()
                .flatMap(roleEntry -> roleEntry.getValue().entrySet().stream()
                        .map(runeEntry -> newRuneEntity(championId, roleEntry.getKey(), patch, q, runeEntry.getKey(),
                                runeEntry.getValue())))
                .collect(Collectors.toList());
        List<ChampionSpellPairStat> spellEntities = spellCounts.entrySet().stream()
                .flatMap(roleEntry -> roleEntry.getValue().entrySet().stream()
                        .map(spellEntry -> newSpellEntity(championId, roleEntry.getKey(), patch, q, spellEntry.getKey(),
                                spellEntry.getValue())))
                .collect(Collectors.toList());
        ...
        log.info("Aggregation {} done in {}ms. items={}, runes={}, spells={}", championId,
                (System.currentTimeMillis() - t0), distinctItems, distinctRunes, distinctSpells);
    }
     ...
     */
    private static <K> void incrementCounter(Map<K, StatCounter> bucket, K key, boolean win) {
        bucket.computeIfAbsent(key, k -> new StatCounter()).increment(win);
    }
     ...
     * @return Normalized role (TOP, JUNGLE, MIDDLE, BOTTOM, UTILITY) or null for
     *         invalid/NONE
     */
     ...
     * @param data       StatCounter containing [totalCount, winCount]
     * @return New ChampionItemStat entity
     */
    private ChampionItemStat newItemEntity(String championId, String role, String patch, int queueId, int itemId,
            StatCounter data) {
     ...
     */
    private ChampionRuneStat newRuneEntity(String championId, String role, String patch, int queueId, String key,
            StatCounter data) {
     ...
     */
    private ChampionSpellPairStat newSpellEntity(String championId, String role, String patch, int queueId, String key,
            StatCounter data) {
        String[] parts = key.split("\\|");
