...
// Import für AntPathRequestMatcher - matched Request-Pfade mit Ant-Pattern
...
@EnableWebSecurity
// Öffentliche Klasse: Konfiguration der Sicherheitsmechanismen (CSRF, Security-Header, etc.)
public class SecurityConfig {

    // Konstante: Array mit allen öffentlich zugänglichen Endpoints (ohne Authentifizierung)
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http, RateLimitingFilter rateLimitingFilter) throws Exception {
        ...
        http
            ...
            .csrf(csrf -> csrf
                ...
                // NOTIZ: Dieser POST-Endpoint wird von einem automatischen Scheduler aufgerufen
            ...
            .headers(headers -> headers
                ...
                // SICHERHEIT: Verhindert MIME-Type-Sniffing (Browser interpretiert Content-Type strikt)
                .contentTypeOptions(Customizer.withDefaults())
                // X-Frame-Options: SAMEORIGIN
                // SICHERHEIT: Erlaubt iFrames nur von gleicher Origin (Clickjacking-Schutz)
                .frameOptions(frame -> frame.sameOrigin())
                // Referrer-Policy: strict-origin-when-cross-origin
                // DATENSCHUTZ: Sendet nur Origin bei Cross-Origin-Requests (nicht voller Pfad)
                ...
                .httpStrictTransportSecurity(hsts -> hsts
                    ...
                    // Preload ist deaktiviert (würde Eintrag in Browser-Preload-Liste erfordern)
                    .preload(false)
                    // Max-Age: 15.552.000 Sekunden = 180 Tage
                    .maxAgeInSeconds(15552000)
                )
                // Permissions-Policy: Deaktiviert Browser-Features
                // SICHERHEIT: Verhindert Zugriff auf Geolocation, Mikrofon, Kamera, Payment-APIs
            ...
            .authorizeHttpRequests(auth -> auth
                ...
                // Erlaubt alle anderen Requests ebenfalls (öffentliche API)
    ...
    // @Bean: Registriert FilterRegistrationBean für manuelle Rate-Limiting-Filter-Kontrolle
    // Deaktiviert automatische Registrierung des RateLimitingFilter durch Spring Boot
    @Bean
    ...
    // WICHTIG: Behandelt X-Forwarded-For, X-Forwarded-Proto, X-Forwarded-Host Header
    @Bean
