 ...
 * Provides database access for champion rune configuration statistics aggregated from high-ELO matches.
 *
 * This repository manages data about which rune combinations are most commonly used on each champion.
 * Rune data includes the primary rune tree, secondary rune tree, and keystone rune, tracked per
 * patch version, queue type, and role. Statistics help players choose optimal rune setups.
 *
 * Custom query methods follow Spring Data JPA naming conventions to find specific rune configurations
...
public interface ChampionRuneStatRepository extends JpaRepository<ChampionRuneStat, Long> {
     ...
     * Used to check if a specific rune setup already exists before updating statistics.
     *
     ...
     * @return Optional containing the matching ChampionRuneStat if found, empty otherwise
     */
    ...
    /**
     * Finds the top 10 most popular rune configurations for a champion in a specific role.
     * Results are ordered by count (descending) to show most frequently used runes first.
     *
     * @param championId The champion identifier
     * @param role The lane role
     * @param patch The patch version
     * @param queueId The queue type
     * @return List of up to 10 rune stats ordered by usage count (most popular first)
     */
    ...
    /**
     * Finds the top 10 most popular rune configurations for a champion across all roles.
    ...
    /**
     * Deletes all rune configuration records for a champion in a specific patch and queue.
