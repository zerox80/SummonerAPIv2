 ...
 *
 * This repository tracks the historical LP (League Points) and rank changes for
 * players
 * over time. Each record captures a player's rank, tier, LP, wins, and losses
 * at a specific
 * point in time, allowing the application to calculate LP gains/losses between
 * matches and
 * display rank progression.
 *
 * Custom query methods use timestamps to find LP records before or after
 * specific matches,
...
public interface PlayerLpRecordRepository extends JpaRepository<PlayerLpRecord, Long> {
         ...
         * @return Optional containing the most recent LP record before the timestamp,
         *         empty if none found
         */
         ...
         * @return Optional containing the earliest LP record at or after the timestamp,
         *         empty if none found
         */
         ...
         * @param puuid     The player's unique identifier (PUUID)
         * @param queueType The ranked queue type (RANKED_SOLO_5x5 or RANKED_FLEX_SR)
         * @return List of all LP records for the player in the queue, ordered by
        ...
        /**
         * Finds LP records for a player in a specific queue within a time range.
         * Used to fetch only relevant records for LP change calculation.
         *
         * @param puuid     The player's PUUID
         * @param queueType The queue type
         * @param start     The start timestamp (inclusive)
         * @param end       The end timestamp (inclusive)
         * @return List of LP records within the time range, ordered by timestamp
         *         ascending
         */
        List<PlayerLpRecord> findByPuuidAndQueueTypeAndTimestampBetweenOrderByTimestampAsc(String puuid,
                        String queueType, Instant start, Instant end);
}
