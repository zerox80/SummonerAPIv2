 ...
 * Provides database access for champion summoner spell pair statistics
 * aggregated from high-ELO matches.
 *
 * This repository manages data about which summoner spell combinations (e.g.,
 * Flash + Ignite,
 * Flash + Teleport) are most commonly used on each champion. Statistics are
 * tracked per
 * patch version, queue type, and role to help players choose optimal summoner
 * spells.
 *
 * Custom query methods follow Spring Data JPA naming conventions to find
 * specific spell pairs
...
public interface ChampionSpellPairStatRepository extends JpaRepository<ChampionSpellPairStat, Long> {
     ...
     * Used to check if a specific spell combination already exists before updating
     * statistics.
     *
     ...
     * @return Optional containing the matching ChampionSpellPairStat if found,
     *         empty otherwise
     */
    ...
    /**
     * Finds the top 10 most popular summoner spell pairs for a champion in a
     * specific role.
     * Results are ordered by count (descending) to show most frequently used spell
     * pairs first.
     *
     ...
     * @return List of up to 10 spell pair stats ordered by usage count (most
     *         popular first)
     */
    List<ChampionSpellPairStat> findTop10ByChampionIdAndRoleAndPatchAndQueueIdOrderByCountDesc(String championId,
            String role, String patch, int queueId);

    /**
     * Finds the top 10 most popular summoner spell pairs for a champion across all
     * roles.
     ...
     *         popular first)
     */
    List<ChampionSpellPairStat> findTop10ByChampionIdAndPatchAndQueueIdOrderByCountDesc(String championId, String patch,
            int queueId);

    /**
     * Deletes all summoner spell pair records for a champion in a specific patch
     * and queue.
    ...
    @org.springframework.data.jpa.repository.Query("DELETE FROM ChampionSpellPairStat c WHERE c.championId = :championId AND c.patch = :patch AND c.queueId = :queueId")
